...
describe('Login tests.', () => {
  ...
  it('Should not log with email not found.', async () => {
    // Arrange
    sinon.stub(UserModel.prototype, "findByEmail").returns(Promise.resolve(null));
    const body = {
  ...
  describe('Route /login/role.', () => {
    ...
    it('Should not return role with invalid token.', async () => {
      // Arrange
      const serviceResponse: ServiceResponse<IRole> = { status: 'UNAUTHORIZED', data: { message: 'Token must be a valid token'}}
      sinon.stub(UserService.prototype, 'getRole').resolves(serviceResponse)
      // Act
      chaiHttpResponse = await chai.request(app).get('/login/role').set('authorization', 'not valid token');
      // Assert
      expect(chaiHttpResponse.status).to.equal(401)
    ...
    it('Should not return role when token is not found.', async () => {
      // Arrange
      const serviceResponse: ServiceResponse<IRole> = { status: 'UNAUTHORIZED', data: { "message": "Token not found" }}
      sinon.stub(UserService.prototype, 'getRole').resolves(serviceResponse)
      // Act
